...
public sealed partial class InstagramContentProvider : IContentProvider
{
    ...
    public InstagramContentProvider(
        InstagramWebClient client,
        ILogger<InstagramContentProvider> logger
    )
    {
    ...
    )
    {
        ...
        using var response = await this._client.GetInstagramPostAsync(
            identifier,
            cancellationToken
        );
        ...
        var streamCopy = new MemoryStream(
            response.Content.Headers.ContentLength != null
                ? (int)response.Content.Headers.ContentLength
                : 32_768
        );
        ...
        if (
            shortcode == null
            || !shortcode.Equals(identifier.Id, StringComparison.OrdinalIgnoreCase)
        )
        {
            var restrictionResult = await this.HasRestrictedProfileAsync(
                identifier,
                cancellationToken
            );
            ...
            this._logger.LogWarning(
                "The expected shortcode {InstagramShortCode} was not found.",
                identifier.Id
            );
            return new(RequestContentResult.NotFound);
        }
        ...
        if (isVideo)
        {
            ...
            var video = await this.ExtractVideoFromNode(shortcodeMedia.Value, cancellationToken);
            content.Videos.Add(video);
        }

        var edgeSidecarToChildren = shortcodeMedia.Value.GetPropertyOrNull(
        ...
        if (edgeSidecarToChildren != null)
        {
            ...
            foreach (var edge in edgeSidecarToChildren.Value.GetProperty("edges").EnumerateArray())
            {
                ...
                {
                    var video = await this.ExtractVideoFromNode(node.Value, cancellationToken);
                    content.Videos.Add(video);
                }
            }
        }
        ...
        var likes = shortcodeMedia
            .Value.GetProperty("edge_media_preview_like")
            .GetProperty("count")
            .GetInt32();
    ...
    private async ValueTask<Video> ExtractVideoFromNode(
        JsonElement videoNode,
        CancellationToken cancellationToken
    )
    {
        ...
        {
            DisplayUrl = new Uri(
                videoNode.GetProperty("display_url").GetString()!,
                UriKind.Absolute
            ),
            Duration = videoNode.TryGetProperty("video_duration", out var videoDuration)
        ...
        var (contentLength, contentType) =
            await this._client.GetVideoContentLengthAndMediaTypeAsync(videoUri, cancellationToken);
    ...
    [GeneratedRegex(
        @"""media_id"":""(?<mediaId>\d+)"",""media_owner_id"":""(?<mediaOwnerId>\d+)""",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex MediaOwnerIdRegex();

    private async ValueTask<RequestContentResult?> HasRestrictedProfileAsync(
        InstagramIdentifier identifier,
        CancellationToken cancellationToken
    )
    {
        using var response = await this._client.GetInstagramResponseAsync(
            identifier.Url,
            cancellationToken
        );
        ...
        using var rulingResponse = await this._client.GetInstagramResponseAsync(
