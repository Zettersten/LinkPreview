...
namespace LinkPreview.Polyfills.Squidlr.Instagram.Utilities;

public static partial class UrlUtilities
{
    [GeneratedRegex(
        @"^https?:\/\/[www\.]*instagram\.com\/[\S]*[p|tv|reel|stories]{1}[\/\S]?\/(?<id>[a-zA-Z0-9_-]+).*?",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex InstagramUrlRegex();

    public static bool TryGetInstagramIdentifier(
        string url,
        [NotNullWhen(true)] out InstagramIdentifier? identifier
    )
    {
    ...
    public static InstagramIdentifier GetInstagramIdentifier(string url)
    {
        if (!TryGetInstagramIdentifier(url, out var identifier))
            throw new ArgumentException(
                "The value represents no valid Instagram URL.",
                nameof(url)
            );
