...
public sealed partial class TiktokContentProvider : IContentProvider
{
    ...
    )
    {
        ...
        var videoDetail = root.GetPropertyOrNull("__DEFAULT_SCOPE__")
        ...
        content.CreatedAtUtc = DateTimeOffset.FromUnixTimeSeconds(
            Convert.ToInt64(
                itemStruct.GetProperty("createTime").GetString(),
        ...
        content.CollectCount = Convert.ToInt32(
            stats.GetProperty("collectCount").GetString(),
            CultureInfo.InvariantCulture
        );
        ...
        foreach (var bitrateInfo in videoElement.GetProperty("bitrateInfo").EnumerateArray())
        {
            var videoSource = new VideoSource
            {
                ...
                ContentLength = Convert.ToInt32(
                    bitrateInfo.GetProperty("PlayAddr").GetProperty("DataSize").GetString(),
                    CultureInfo.InvariantCulture
                )
            };
