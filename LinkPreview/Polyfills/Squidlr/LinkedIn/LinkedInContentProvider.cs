...
public sealed partial class LinkedInContentProvider : IContentProvider
{
    ...
    public LinkedInContentProvider(
        LinkedInWebClient client,
        ILogger<LinkedInContentProvider> logger
    )
    {
    ...
    )
    {
        ...
        using var response = await this._client.GetLinkedInPostAsync(identifier, cancellationToken);
        ...
        var scriptNode = htmlDoc
            .DocumentNode.Descendants("script")
            .FirstOrDefault(n =>
                n.HasAttributes
                && n.Attributes.Any(a => a.Name == "type" && a.Value == "application/ld+json")
            );
        if (scriptNode == null)
        {
            this._logger.LogWarning("The expected script node was not found.");
        ...
        if (contentUrl == null)
        {
            this._logger.LogWarning("The expected contentUrl property was not found.");
            return new(RequestContentResult.NoVideo);
        }
        ...
        var (contentLength, mediaType) = await this._client.GetVideoContentLengthAndMediaTypeAsync(
            videoFileUrl,
            cancellationToken
        );
        ...
        {
            this._logger.LogWarning("Could not obtain the LinkedIn creator property.");
        }

        var reactionsNode = htmlDoc
            .DocumentNode.Descendants("a")
            .FirstOrDefault(n =>
                n.HasAttributes && n.Attributes.Any(a => a.Name == "data-num-reactions")
            );
        ...
        var commentsNode = htmlDoc
            .DocumentNode.Descendants("a")
            .FirstOrDefault(n =>
                n.HasAttributes && n.Attributes.Any(a => a.Name == "data-num-comments")
            );
