...
namespace LinkPreview.Polyfills.Squidlr.Facebook.Utilities;

public static partial class UrlUtilities
{
    [GeneratedRegex(
        @"^https?:\/\/[www\.|m\.|web\.]+facebook\.com\/((\S+\/videos|reel\/|groups\/\d+\/(posts|permalink){1}\/)|((video.php|watch\/?){1}\?v=)){1}(\S*\/)?(?<id>\d+).*?",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex FacebookUrlRegex();

    [GeneratedRegex(
        @"^https?:\/\/[www\.|m\.]+facebook\.com\/share\/\w{1}\/(?<id>[\s\S][^\?\/]+)",
        RegexOptions.IgnoreCase
    )]
    ...
    [GeneratedRegex(
        @"^https?:\/\/[www\.|m\.]+facebook\.com\/story.php\?story_fbid=[\d]+&id=(?<id>[\d]+){1}",
        RegexOptions.IgnoreCase
    )]
    private static partial Regex FacebookStoryUrlRegex();

    public static bool TryGetFacebookIdentifier(
        string url,
        [NotNullWhen(true)] out FacebookIdentifier? identifier
    )
    {
    ...
    [
        FacebookUrlRegex(),
        FacebookShareUrlRegex(),
        FacebookWatchUrlRegex(),
        FacebookStoryUrlRegex()
    ];
