...
public sealed class TwitterWebClient
{
    ...
    private readonly ILogger<TwitterWebClient> logger;

    public TwitterWebClient(
        IHttpClientFactory httpClientFactory,
        IMemoryCache memoryCache,
        ILogger<TwitterWebClient> logger
    )
    {
        this.clientFactory =
    ...
    public async ValueTask<long?> GetVideoContentLengthAsync(
        Uri videoFileUri,
    ...
    public async Task<Result<Stream, RequestContentResult>> GetTweetDetailStreamAsync(
        TweetIdentifier tweet,
        CancellationToken cancellationToken
    )
    {
        var guestToken = await this.GetOrCreateGuestTokenAsync(tweet, cancellationToken);
        if (guestToken == null)
        {
    ...
    private async ValueTask<string?> GetOrCreateGuestTokenAsync(
        TweetIdentifier tweet,
        CancellationToken cancellationToken
    )
    {
        try
        {
            return await this.memoryCache.GetOrCreateAsync(
                ...
                async (cacheEntry) =>
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        this.logger.LogWarning("Twitter guest token API is probably deprecated.");
                        ...
                        guestToken =
                            GuestTokenUtilities.ExtractGuestToken(document)
                            ?? throw new InvalidOperationException(
                                "Guest token not found in HTML document"
                            );
                    ...
                    guestToken =
                        json.RootElement.GetProperty("guest_token").GetString()
        ...
        {
            this.logger.LogError(e, "Could not request new Twitter guest token.");
            return null;
        }
    }
}
